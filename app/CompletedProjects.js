    ...
    const fetchMyPosts = async () => {
      ...
      try {
        ...
        if (userData.role === 'citizen') {
          queryCondition = query(collection(db, "posts"), where("userId", "==", user.uid), where("status", "==", 'Completed'));
        } else if (userData.role === 'ministry') {
          queryCondition = query(collection(db, "posts"), where("Ministry", "==", userData.fullname), where("status", "==", 'Completed'));
        } else if (userData.role === 'Governor') {
            queryCondition = query(collection(db, "posts"),  where("status", "==", 'Completed'));
        }
